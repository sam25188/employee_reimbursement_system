...
public class SessionController {
    ...
    public void login(Context context){
        ...
        User userFromDb = userService.getByUsername(credentials.getUsername(), credentials.getPassword());

        if(userFromDb == null) {
            jsonResponse = new JsonResponse(false, "invalid username or password", null);
        }else{
            context.sessionAttribute("user", userFromDb);
            jsonResponse = new JsonResponse(true, "login successful welcome", userFromDb);
        }
